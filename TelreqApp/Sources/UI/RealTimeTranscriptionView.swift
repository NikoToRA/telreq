...
struct RealTimeTranscriptionView: View {
    ...
    private var transcriptionDisplayArea: some View {
        ScrollViewReader { proxy in
            ...
            .onChange(of: viewModel.liveSegmentId) { newSegmentId in
...
class RealTimeTranscriptionViewModel: ObservableObject {
    ...
    func initialize() async {
        do {
            ...
            guard await checkPermissions() else {
                showError("マイクアクセス権限が必要です", canRetry: false)
                return
            }
            ...
            logger.error("Failed to initialize real-time transcription: \(error.localizedDescription)")
            showError("転写機能の初期化に失敗しました", canRetry: true)
        }
    }
    ...
    func saveTranscription() async {
        ...
        do {
            ...
            let callData = StructuredCallData(
                ...
                participantNumber: "リアルタイム転写",
                audioFileUrl: "",
                transcriptionText: fullText,
                summary: CallSummary(
                    keyPoints: [],
                    summary: "リアルタイム転写セッション",
                    duration: transcriptionDuration,
                    participants: ["ユーザー"],
                ...
                metadata: CallMetadata(
                    ...
                    transcriptionMethod: currentMethod,
                    language: "ja-JP",
                    confidence: averageConfidence ?? 0,
                    startTime: Date().addingTimeInterval(-transcriptionDuration),
                    endTime: Date(),
            ...
            _ = try await serviceContainer?.offlineDataManager.saveCallData(callData)
    ...
    private func startTimers() {
        ...
        statisticsTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: true) { _ in
            Task { @MainActor in
                ...
                self.simulateTranscriptionSegment() // デモ用
            }
        }
    }
    ...
    /// 転写セグメントをシミュレート（デモ用）
    private func simulateTranscriptionSegment() {
        guard isTranscribing && !isPaused else { return }
        
        let sampleTexts = [
            "こんにちは、今日はお忙しい中お時間をいただきありがとうございます。",
            "来週の会議の件についてお話したいと思います。",
            "資料の準備はいかがでしょうか。",
            "予定を確認させていただきたいのですが。",
            "次回までに必要な作業を整理しましょう。"
        ]
        ...
        let newSegment = TranscriptionSegment(
            timestamp: Date(),
            text: sampleTexts.randomElement() ?? "テストテキスト",
            confidence: Double.random(in: 0.7...0.95),
            speakerId: speakerIdentification ? "Speaker1" : nil,
            isLive: true
        )
        
        // 前のライブセグメントを通常セグメントに変更
        if let liveId = liveSegmentId,
           let index = transcriptionSegments.firstIndex(where: { $0.id == liveId }) {
            let updatedSegment = TranscriptionSegment(
                timestamp: transcriptionSegments[index].timestamp,
                text: transcriptionSegments[index].text,
                confidence: transcriptionSegments[index].confidence,
                speakerId: transcriptionSegments[index].speakerId,
                isLive: false
            )
            transcriptionSegments[index] = updatedSegment
        }
        
        transcriptionSegments.append(newSegment)
        liveSegmentId = newSegment.id
        currentConfidence = newSegment.confidence
        
        // 最大セグメント数を超えた場合は古いものを削除
