...
class SharingViewModel: ObservableObject {
    ...
    func loadInitialData() async {
        ...
        do {
            ...
            try await (receivedTask, sentTask, requestsTask)
    ...
    func searchUsers(_ query: String) async {
        ...
        do {
            ...
            logger.info("Found \(searchResults.count) users for query: \(query)")
        } catch {
    ...
    func acceptSharingRequest(_ request: SharingRequest) async {
        do {
            ...
            await loadReceivedSharedRecords()
    ...
    func rejectSharingRequest(_ request: SharingRequest) async {
        do {
            ...
            logger.error("Failed to reject sharing request: \(error.localizedDescription)")
            showError("共有リクエストの拒否に失敗しました")
        }
    }
    ...
    func removeSharedRecord(_ sharedRecord: SharedCallRecord) async {
        do {
            ...
            logger.error("Failed to remove shared record: \(error.localizedDescription)")
            showError("共有記録の削除に失敗しました")
        }
    }
