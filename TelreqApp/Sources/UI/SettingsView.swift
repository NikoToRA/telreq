...
struct SettingsView: View {
    ...
    private var transcriptionSettingsSection: some View {
        Section("転写設定") {
            ...
            .onChange(of: viewModel.selectedTranscriptionMethod) { newValue in
                Task {
            ...
            Toggle("通話開始時に自動転写", isOn: $viewModel.autoStartTranscription)
                .onChange(of: viewModel.autoStartTranscription) { newValue in
                    Task {
            ...
            Toggle("リアルタイム転写", isOn: $viewModel.realtimeTranscription)
                .onChange(of: viewModel.realtimeTranscription) { newValue in
                    Task {
            ...
            Toggle("自動要約生成", isOn: $viewModel.autoGenerateSummary)
                .onChange(of: viewModel.autoGenerateSummary) { newValue in
                    Task {
    ...
    private var audioSettingsSection: some View {
        Section("音声設定") {
            ...
            .onChange(of: viewModel.recordingQuality) { newValue in
                Task {
            ...
            Toggle("ノイズリダクション", isOn: $viewModel.noiseReduction)
                .onChange(of: viewModel.noiseReduction) { newValue in
                    Task {
            ...
            Toggle("音声ファイルを保存", isOn: $viewModel.saveAudioFiles)
                .onChange(of: viewModel.saveAudioFiles) { newValue in
                    Task {
            ...
            Toggle("音声レベル表示", isOn: $viewModel.showAudioLevels)
                .onChange(of: viewModel.showAudioLevels) { newValue in
    ...
    private var privacySettingsSection: some View {
        Section("プライバシー設定") {
            ...
            Toggle("データ暗号化", isOn: $viewModel.encryptionEnabled)
                .onChange(of: viewModel.encryptionEnabled) { newValue in
                    Task {
            ...
            Toggle("Secure Enclave使用", isOn: $viewModel.useSecureEnclave)
                ...
                .onChange(of: viewModel.useSecureEnclave) { newValue in
                    Task {
            ...
            .onChange(of: viewModel.autoDeletePeriod) { newValue in
                Task {
            ...
            Toggle("アプリ起動時にパスコード", isOn: $viewModel.requirePasscode)
                .onChange(of: viewModel.requirePasscode) { newValue in
                    Task {
    ...
    private var connectionSettingsSection: some View {
        Section("接続設定") {
            // Azure接続設定
            NavigationLink(destination: AzureSettingsView(viewModel: viewModel)) {
                HStack {
                    Text("Azure OpenAI設定")
                    Spacer()
                    ConnectionStatusIndicator(status: viewModel.azureConnectionStatus)
                }
            }
            ...
            Toggle("自動再接続", isOn: $viewModel.autoReconnect)
                .onChange(of: viewModel.autoReconnect) { newValue in
...
class SettingsViewModel: ObservableObject {
    ...
    private func loadStorageUsage() async {
        do {
            storageUsage = try await serviceContainer?.azureStorageService.getStorageUsage()
        } catch {
    ...
    func clearCache() async {
        do {
            try await serviceContainer?.offlineDataManager.clearCache()
    ...
    func syncOfflineData() async {
        ...
        do {
            try await serviceContainer?.azureStorageService.syncOfflineData()
            await loadStorageUsage() // 使用量を再読み込み
    ...
    func showDebugInfo() {
        ...
        serviceContainer?.printDebugInfo()
    }
    
    func resetAllSettings() async {
        do {
            #if DEBUG
            try serviceContainer?.resetAllData()
